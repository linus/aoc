 ...
 *
 * @param {number[]} numbers The numbers making up the diagnostic report
...
function part1(numbers) {
  const { gamma, epsilon } = findCommon(numbers);
...
function part2(numbers) {
  let { gamma } = findCommon(numbers);
  let oxy = numbers.find((number) => (number ^ gamma) === 0);
  let co2 = numbers.find((number) => (number ^ gamma) === 1);

  return oxy * co2;
}

/**
 * Given a list of numbers, finds the most common bit per significant digit
 *
 * @param {number[]} numbers - The numbers to scan
 * @returns {number} The most common bit in each position
 * @example findCommon([
 *   0b00100,
 *   0b11110,
 *   0b10110,
...
function findCommon(numbers) {
  const magnitude = getMagnitude(numbers);
  const bitString = numbers
    .reduce(
      (result, n) => {
        for (let i = 0; i < magnitude; i++) {
          let bit = (n & (1 << i)) > 0 ? 1 : 0;
          result[magnitude - i - 1][bit]++;
        }
        return result;
      },
      Array(magnitude)
  ...
  const epsilon = gamma ^ (pow(2, magnitude) - 1);

  return { gamma, epsilon };
}

/**
 * Returns the magnitude of the largest number
 * @param {number[]} numbers
 * @returns {number}
 * @example getMagnitude([
 *   0b00100,
 *   0b11110,
 *   0b10110,
 *   0b10111,
 ...
 * //=> 5
 */
function getMagnitude(numbers) {
  const largest = numbers
    .slice(0)
    .sort((a, b) => a - b)
    .pop();

  return (log2(largest) + 1) >> 0;
}
